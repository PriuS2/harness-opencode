...
"다운로드 폴더에 있는 Q4_매출_최종_v2.xlsx에서 C열(매출)과 D열(비용)을
...
Task 결과에서 total_tokens와 duration_ms를 **즉시** 저장한다. 이 데이터는 결과 수신 시점에만 접근 가능하고 이후 복구할 수 없다.
...
- 시간/토큰 트레이드오프 (스킬이 품질은 높지만 비용도 높이는 경우)
